...
import { cardModelTopic, pagesData } from '@/constants'
import Details from '@/app/components/Details';
import BottomImage from '@/app/components/BottomImage';

const node = pagesData[5];

const Node = () => {
  return (
    <section className='relative w-full min-h-screen h-full bg-gray-100 '>
        <div className='relative w-full h-3/4 overflow-hidden  '>
            <img src={node.mainImage} alt="" className='w-full h-screen object-cover  ' />
            <div className='absolute inset-0 bg-black opacity-45' />
        </div>

        <div className='absolute top-52 xl:top-[15vh] xl:left-20'>
            <Card className='w-[50vh] xl:w-[65vh] h-full shadow-none p-7 md:pt-42 bg-transparent rounded-3xl text-white border-none '>
                <CardHeader>
                    <CardTitle>
                        <h1 className='text-2xl md:text-6xl font-noto-bold'>{node.title}</h1>
                    </CardTitle>
                </CardHeader>
                <CardContent>
        ...
        <Details detail={node.usable} />



        <BottomImage bottomImage={node.bottomImage} example={node.example} />
    </section>
  )
}
